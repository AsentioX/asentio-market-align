...
function generateRecommendations(sessions: Session[], events: AnalyticsEvent[]): string[] {
  ...
  if (emailClicks > ctaClicks)
    recs.push('More visitors click email directly than CTA buttons — make the email link more prominent.');
...
export default function AnalyticsDashboard() {
  ...
  const engagedSessions = sessions.filter((s) =>
    events.filter((e) => e.session_id === s.id && e.event_type === 'page_view').length >= 2
  ).length;
  ...
  const convDailyData = dailyData.map((d) => ({
    ...d,
    conversions: convDailyMap[Object.keys(convDailyMap).find((k) => k.slice(5) === d.date) || ''] || 0,
  }));
  ...
  const recentConversions = sessions.filter((s) => s.converted).slice(0, 10).map((s) => ({
    ...s,
    trigger: events.find((e) => e.session_id === s.id && ['email_click', 'form_submit', 'cta_click'].includes(e.event_type)),
  }));
  ...
  // Sessions that engaged with the directory
  const directorySessions = sessions.filter((s) =>
    events.some((e) => e.session_id === s.id && (
      e.event_type === 'directory_view' ||
      e.event_type === 'directory_tab_view' ||
      e.page_path.startsWith('/xr-directory')
    ))
  );
    ...
    <div className="space-y-6">
          ...
          {([7, 30, 90] as const).map((d) => (
            <button
              ...
              className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
                range === d ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground hover:bg-muted/80'
              }`}
            >
              {d}d
            </button>
      ...
      <Tabs defaultValue="conversion">
        ...
        <TabsContent value="directory" className="space-y-8">
                ...
                {tabData.length ? (
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={tabData}>
                      ...
                      <YAxis allowDecimals={false} tick={{ fontSize: 11 }} />
                      <Tooltip />
                      <Bar dataKey="value" fill="#1d5ba8" name="Views" />
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
          ...
          {/* Products viewed before conversion */}
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
            ...
            <CardContent>
              {topAssistingProducts.length ? (
                <div className="space-y-2">
                  {topAssistingProducts.map(({ name, assists }) => (
                    <div key={name} className="flex items-center justify-between py-1.5 border-b last:border-0">
                      <span className="text-sm font-medium text-foreground">{name}</span>
                      <Badge className="bg-green-500/10 text-green-700 border-green-200">
                        {assists} assisted conversion{assists !== 1 ? 's' : ''}
                      </Badge>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-muted-foreground">No conversion assists yet — appears when a product page is viewed in a session that converts.</p>
              )}
              <p className="text-xs text-muted-foreground mt-4 pt-3 border-t">
                "Assisted conversion" = product detail was viewed in a session that ended with an email click, CTA click, or form submission.
              </p>
            </CardContent>
          </Card>
                ...
                {(() => {
                  ...
                  if (directorySessions.length === 0) {
                    ...
                    if (topProducts.length > 0) {
                      insights.push(`"${topProducts[0].name}" is the most-viewed product — use it as a case study anchor or reference it in outreach to demonstrate market awareness.`);
                    }
                    if (tabData.length > 0) {
                      const topTab = [...tabData].sort((a, b) => b.value - a.value)[0];
                      insights.push(`The "${topTab.name}" tab is the most browsed section — make sure it has strong content and a visible contact CTA.`);
                    }
                  }
