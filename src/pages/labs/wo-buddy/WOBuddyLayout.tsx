          ...
          <div className="bg-gradient-to-br from-stone-900/[0.05] to-stone-900/[0.03] rounded-2xl p-4 border border-stone-900/10">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="text-stone-900/40">Signed in as</span>
                <span className="text-stone-900/60">{user.email}</span>
              </div>
              {isAdmin && (
                <Link
          ...
          <div className="flex flex-col items-center">
            ...
            {weather && (
              <div className="flex items-center gap-1.5 -mt-0.5">
                <MapPin className="w-2.5 h-2.5 text-stone-900/30" />
                <span className="text-[9px] text-stone-900/40">{weather.city}</span>
                <span className="text-[10px]">{getWeatherEmoji(weather.code, weather.isDay)}</span>
                <span className="text-[9px] text-stone-900/50 font-medium">{weather.temp}°F</span>
              </div>
            )}
          </div>
          <button onClick={() => setActiveTab('settings')} className="w-8 h-8 rounded-lg bg-stone-900/5 flex items-center justify-center text-stone-900/40 hover:text-stone-900/60">
