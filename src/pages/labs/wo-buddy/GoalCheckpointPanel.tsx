  ...
  return (
    <div className="space-y-2.5">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] text-white/40 uppercase tracking-wider">4-Week Checkpoints</span>
      </div>
      ...
      {next && (
        <div className={`rounded-xl border p-3 space-y-2.5 ${
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              {isDue ? <Sparkles className="w-3.5 h-3.5 text-amber-400" /> : <Clock className="w-3.5 h-3.5 text-white/40" />}
              <span className={`text-xs font-medium ${isDue ? 'text-amber-300' : 'text-white/70'}`}>
                {isDue ? 'Checkpoint due now' : `Next checkpoint ${formatDistanceToNow(new Date(next.scheduled_for), { addSuffix: true })}`}
              </span>
            </div>
            <span className="text-[10px] text-white/30 font-mono">#{next.sequence_number}</span>
          </div>
          <p className="text-[11px] text-white/50 leading-relaxed">
            Measure your current performance now. We'll use this to adjust the rest of your training plan.
            {expectedAtNext !== null && (
              <> Expected pace: <span className="text-white/70 font-medium">~{expectedAtNext} {metricUnit}</span>.</>
            )}
          </p>
          {isDue && (
            <>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-amber-400/40 [&::-webkit-inner-spin-button]:appearance-none"
                />
                <span className="flex items-center text-[11px] text-white/40 px-1">{metricUnit}</span>
              </div>
              <input
                type="text"
                value={note}
                ...
                className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-[11px] text-white placeholder:text-white/20 focus:outline-none focus:border-amber-400/40"
              />
              <div className="flex gap-2">
                <button
                  onClick={handleLog}
                  disabled={!inputValue || submitting}
                  className="flex-1 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold text-xs disabled:opacity-40"
                >
                  Log & Adapt Plan
                </button>
                <button
      ...
      {past.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[10px] text-white/30 uppercase tracking-wider">Past check-ins</p>
          {past.slice(-4).map(c => (
            <div key={c.id} className="flex items-center gap-2 text-[11px] bg-white/[0.02] rounded-lg px-2.5 py-1.5">
              {c.status === 'logged' ? (
                <CheckCircle2 className="w-3 h-3 text-emerald-400 shrink-0" />
              ) : (
                <MinusCircle className="w-3 h-3 text-white/20 shrink-0" />
              )}
              <span className="text-white/40">{format(new Date(c.scheduled_for), 'MMM d')}</span>
              {c.status === 'logged' && c.measured_value !== null && (
                <span className="text-white/70 font-medium">
                  {c.measured_value} {metricUnit}
                </span>
              )}
              {c.status === 'skipped' && <span className="text-white/30 italic">skipped</span>}
              {c.note && <span className="text-white/30 truncate">· {c.note}</span>}
            </div>
          ))}
        </div>
      )}
