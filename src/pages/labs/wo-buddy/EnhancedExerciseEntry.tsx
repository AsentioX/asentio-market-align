    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px] text-white/40">{exercise.subcategory} • {exercise.entryType === 'sets' ? `${sets.length} sets` : exercise.entryType}</p>
        </div>
      </div>
      ...
      >
        ...
        {showWhy ? <ChevronUp className="w-3 h-3 text-white/20" /> : <ChevronDown className="w-3 h-3 text-white/20" />}
      </button>
      {showWhy && (
        <div className="px-3 pb-1 space-y-2">
          <p className="text-xs text-white/50 leading-relaxed">{exercise.whyItMatters}</p>
          <div className="flex flex-wrap gap-1">
            {exercise.linkedDrivers.map(d => (
              <span key={d} className="text-[9px] px-2 py-0.5 rounded-md bg-white/5 text-white/35 border border-white/5">{d}</span>
            ))}
          </div>
        </div>
      )}
      ...
      {exercise.entryType === 'sets' && (
        <div className="space-y-2">
          {/* Column headers */}
          <div className="flex items-center gap-2 px-2 text-[9px] text-white/30 uppercase tracking-wider">
          ...
          {sets.map((set, idx) => (
            <div key={idx} className="flex items-center gap-2 bg-white/[0.03] rounded-xl border border-white/[0.06] p-2">
              <span className="w-8 text-center text-xs font-bold text-white/40">{set.setNumber}</span>
              {exercise.defaultMetrics.filter(m => m.key !== 'sets').map(m => (
                <div key={m.key} className="flex-1">
                  <input
                    ...
                    placeholder={m.label}
                    className="w-full bg-white/5 border border-white/5 rounded-lg px-2 py-2 text-center text-sm font-semibold text-white focus:outline-none focus:border-emerald-500/30 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                  />
                </div>
              ))}
              <button onClick={() => removeSet(idx)} className="w-8 h-8 rounded-lg flex items-center justify-center text-white/20 hover:text-red-400 transition-colors">
                <Trash2 className="w-3.5 h-3.5" />
              </button>
            </div>
          ))}

          <button onClick={addSet} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-dashed border-white/10 text-xs text-white/30 hover:text-white/50 hover:border-white/20 transition-colors">
      ...
      {exercise.entryType === 'intervals' && (
        <div className="space-y-2">
          ...
          <button onClick={addSet} className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-dashed border-white/10 text-xs text-white/30 hover:text-white/50 transition-colors">
      ...
      {(exercise.entryType === 'simple' || exercise.entryType === 'duration') && (
        <div className="grid grid-cols-2 gap-2">
          {exercise.defaultMetrics.map(m => (
            <div key={m.key} className="bg-white/[0.03] rounded-xl border border-white/[0.06] p-3">
              <label className="text-[9px] text-white/30 uppercase tracking-wider block mb-1.5">{m.label}{m.unit ? ` (${m.unit})` : ''}</label>
              <input
                type="number"
                value={simpleValues[m.key] || ''}
                onChange={e => setSimpleValues(prev => ({ ...prev, [m.key]: Number(e.target.value) }))}
                placeholder="0"
                className="w-full bg-white/5 border border-white/5 rounded-lg px-3 py-2.5 text-lg font-bold text-white text-center focus:outline-none focus:border-emerald-500/30 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
              />
            </div>
          ))}
        </div>
      ...
      <div className="bg-white/[0.03] rounded-xl border border-white/[0.06] p-3.5">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] text-white/30 uppercase tracking-wider">Perceived Effort (RPE)</span>
        ...
        <input
          ...
          className="w-full h-2 bg-white/10 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-emerald-400 [&::-webkit-slider-thumb]:shadow-lg"
        />
      ...
      {showNotes && (
        <textarea
          ...
          rows={3}
          className="w-full bg-white/[0.03] border border-white/[0.06] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-emerald-500/30 resize-none"
        />
      )}

      {/* Action buttons */}
      <div className="flex gap-2">
        <button onClick={onCancel} className="flex-1 py-3 rounded-xl bg-white/5 text-white/50 text-sm font-medium active:bg-white/10 transition-colors">
          Cancel
        </button>
        <button onClick={handleSave} className="flex-1 py-3 rounded-xl bg-emerald-500 text-white text-sm font-semibold active:bg-emerald-600 transition-colors flex items-center justify-center gap-1.5">
