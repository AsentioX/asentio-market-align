    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-1.5">
          {STEPS.map(s => (
            <div key={s.id} className="flex-1">
              ...
              />
              <div className={`text-[9px] mt-1 text-center font-medium ${s.id === step ? 'text-emerald-300' : 'text-white/30'}`}>
                {s.label}
              </div>
            </div>
          ))}
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div>
            <h2 className="text-base font-semibold text-white">Tell us about the exercise</h2>
            <p className="text-xs text-white/40 mt-0.5">The basics help us file it correctly.</p>
          </div>
          ...
          <Field label="Exercise name" required error={errors.name}>
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-emerald-500/40"
                maxLength={80}
              />
              <input
                type="text"
                ...
                className="w-14 text-center bg-white/[0.04] border border-white/[0.08] rounded-xl px-2 py-2.5 text-xl"
              ...
              {DIFFICULTY_OPTIONS.map(opt => (
                <button
                  ...
                  className={`py-2 rounded-xl border text-[11px] font-medium transition-all ${
                    difficulty === opt.value ? opt.color : 'bg-white/[0.03] border-white/[0.06] text-white/40'
                  }`}
                >
                  {opt.label}
                </button>
          ...
          <Field label="Description (optional)">
            <textarea
              ...
              rows={2}
              maxLength={240}
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-emerald-500/40 resize-none"
            />
          </Field>
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div>
            <h2 className="text-base font-semibold text-white">Which drivers does it train?</h2>
            <p className="text-xs text-white/40 mt-0.5">Drivers connect this exercise to your goals.</p>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className={`text-xs font-semibold ${selected ? 'text-emerald-300' : 'text-white/80'}`}>{d.name}</div>
                  </div>
          ...
          {drivers.length > 0 && (
            <div className="space-y-2.5">
              ...
              {drivers.map(d => {
                ...
                return (
                  <div key={d.driver} className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-3 space-y-2.5">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-base">{meta?.icon}</span>
                        <span className="text-sm font-semibold text-white">{d.driver}</span>
                      </div>
                      <button
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] uppercase tracking-wider text-white/40">Contribution</span>
                    ...
                    <input
                      ...
                      maxLength={140}
                      className="w-full bg-white/[0.04] border border-white/[0.06] rounded-lg px-2.5 py-1.5 text-[11px] text-white placeholder:text-white/25 focus:outline-none focus:border-emerald-500/30"
                    />
                  </div>
                );
              })}
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div>
            <h2 className="text-base font-semibold text-white">How will you measure it?</h2>
            <p className="text-xs text-white/40 mt-0.5">Pick a preset or add metrics yourself.</p>
          </div>

          {/* Presets */}
          <div className="rounded-xl border border-emerald-500/15 bg-gradient-to-br from-emerald-500/[0.06] to-white/[0.02] p-3 space-y-2">
            <div className="flex items-center gap-1.5">
              <Sparkles className="w-3 h-3 text-emerald-400" />
              <span className="text-[10px] font-semibold uppercase tracking-widest text-white/50">Quick presets</span>
            </div>
            <div className="grid grid-cols-2 gap-1.5">
              {(Object.keys(METRIC_PRESETS) as (keyof typeof METRIC_PRESETS)[]).map(key => (
                <button
                  ...
                  className="text-[11px] py-2 rounded-lg bg-white/[0.04] border border-white/[0.06] text-white/70 hover:bg-emerald-500/10 hover:border-emerald-500/30 hover:text-emerald-300 transition-all capitalize"
                >
                  {key}
                </button>
              ))}
          ...
          <div className="space-y-2">
            {metrics.map(m => (
              <div key={m.id} className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-2.5 space-y-2">
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    value={m.name}
                    ...
                    className="flex-1 bg-white/[0.05] border border-white/[0.06] rounded-lg px-2.5 py-1.5 text-xs text-white placeholder:text-white/25 focus:outline-none focus:border-emerald-500/30"
                  />
                  <button
                    onClick={() => removeMetric(m.id)}
                    className="w-7 h-7 flex items-center justify-center rounded-lg text-white/30 hover:text-red-400 hover:bg-red-500/10"
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  <select
                    ...
                    className="col-span-1 bg-white/[0.05] border border-white/[0.06] rounded-lg px-2 py-1.5 text-[11px] text-white focus:outline-none focus:border-emerald-500/30"
                  >
                  ...
                  <input
                    ...
                    placeholder="Unit"
                    className="col-span-1 bg-white/[0.05] border border-white/[0.06] rounded-lg px-2 py-1.5 text-[11px] text-white placeholder:text-white/25 focus:outline-none focus:border-emerald-500/30"
                  />
                  <button
            ...
            <button
              onClick={() => addMetric()}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-dashed border-white/15 text-xs text-white/50 hover:text-emerald-300 hover:border-emerald-500/40 transition-all"
            >
      ...
      {step === 4 && (
        <div className="space-y-4">
          <div>
            <h2 className="text-base font-semibold text-white">Connect to your goals</h2>
            <p className="text-xs text-white/40 mt-0.5">Optional — tells us why this exercise matters to you.</p>
          </div>

          {goals.length === 0 ? (
            <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-5 text-center space-y-2">
              <Target className="w-5 h-5 text-white/30 mx-auto" />
              <p className="text-xs text-white/50">No active goals yet.</p>
              <p className="text-[11px] text-white/30">Add a goal in the Goals tab to link exercises.</p>
            </div>
          ) : (
            <div className="space-y-2">
              {goals.map(g => {
                ...
                return (
                  <div key={g.id} className={`rounded-xl border transition-all ${selected ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-white/[0.03] border-white/[0.06]'}`}>
                    <button
                      onClick={() => toggleGoal(g.id)}
                      className="w-full flex items-center gap-2 px-3 py-2.5 text-left"
                    >
                      <div className={`w-4 h-4 rounded-md flex items-center justify-center border ${selected ? 'bg-emerald-500 border-emerald-500' : 'border-white/20'}`}>
                        {selected && <Check className="w-3 h-3 text-white" />}
                      </div>
                      <Target className="w-3.5 h-3.5 text-white/40" />
                      <span className={`text-xs font-medium flex-1 truncate ${selected ? 'text-emerald-200' : 'text-white/70'}`}>{g.name}</span>
                    </button>
                    {selected && (
      ...
      {step === 5 && (
        <div className="space-y-4">
          <div>
            <h2 className="text-base font-semibold text-white">Looks good?</h2>
            <p className="text-xs text-white/40 mt-0.5">Review and add to your workout.</p>
          </div>
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-gradient-to-br from-white/[0.05] to-white/[0.02] p-4 space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-white/50 capitalize">{category}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-white/50 capitalize">{difficulty}</span>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {drivers.map(d => (
                  <span key={d.driver} className={`text-[10px] px-2 py-0.5 rounded-full border ${
                    d.isPrimary ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' : 'bg-white/5 text-white/60 border-white/10'
                  }`}>
            ...
            <Section title="Metrics">
              <div className="flex flex-wrap gap-1.5">
                {metrics.map(m => (
                  <span key={m.id} className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-white/60 border border-white/10">
      ...
      <div className="flex items-center gap-2 pt-2">
        {step > 1 && (
          <button
            ...
            className="flex-1 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm font-medium text-white/70 hover:bg-white/[0.07] transition-colors"
          >
            Back
          </button>
        )}
        {step < STEPS.length ? (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-sm font-semibold transition-all ${
              canAdvance
                ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white shadow-lg shadow-emerald-500/20 active:scale-[0.98]'
          ...
          <button
            onClick={handleSave}
            className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-sm font-semibold shadow-lg shadow-emerald-500/20 active:scale-[0.98] transition-all"
          >
...
const Section = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div className="space-y-1.5">
    <div className="text-[10px] font-semibold uppercase tracking-widest text-white/40">{title}</div>
    {children}
  </div>
);
