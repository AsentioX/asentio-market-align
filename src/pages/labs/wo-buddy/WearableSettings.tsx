  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-white/40 mt-1">Pair your watch, ring, or fitness band to sync data automatically.</p>
      </div>

      {/* Connected / saved devices */}
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-widest text-white/50 mb-3">Your Devices</h3>
            ...
            <div key={device.id} className={`rounded-2xl border overflow-hidden transition-all ${
              ...
              <div className="p-4 flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {!device.connected && (
                    <button onClick={() => removeDevice(device.id)} className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center text-white/20 hover:text-red-400 transition-colors">
              ...
              {device.connected && (
                <div className="px-4 pb-3 flex gap-2 flex-wrap">
                  {['Heart Rate', 'Steps', 'Sleep', 'Calories'].map(metric => (
                    <span key={metric} className="text-[9px] bg-white/5 text-white/40 px-2 py-1 rounded-full border border-white/5">
                      {metric} ✓
                    </span>
                  ))}
                </div>
      ...
      {showAvailable && (
        <div className="space-y-2">
          <h3 className="text-xs font-semibold uppercase tracking-widest text-white/50">Available Nearby</h3>
          {availableDevices
            ...
            .map((ad) => (
              <button
                ...
                className="w-full flex items-center gap-3 p-3.5 rounded-2xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] transition-colors text-left"
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-white/30">Tap to pair</p>
                </div>
      ...
      {/* Health integrations */}
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-widest text-white/50 mb-3">Health Integrations</h3>
        <div className="space-y-2">
          {[
