      ...
      <div className="space-y-6">
        <div className="flex flex-col items-center justify-center text-center space-y-5 pt-2">
          ...
          <div>
            ...
            <p className="text-stone-700 text-sm">
              {elapsedSeconds > 0 && <span className="text-stone-800">{formatTimer(elapsedSeconds)} · </span>}
                ...
                return (
                  <div key={d} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-stone-900/5 border border-stone-200">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-stone-600">{info?.description || ''}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {impact.goals.length > 0 && (
          <div className="bg-gradient-to-br from-emerald-500/5 to-emerald-600/[0.02] rounded-2xl p-4 border border-emerald-500/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-stone-700">Goal Impact</span>
            </div>
                ...
                return (
                  <div key={g.id} className="bg-transparent rounded-xl p-3 border border-stone-200">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-stone-600">{g.drivers.join(', ')}</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-stone-700 font-medium tabular-nums">{pct}%</span>
                    </div>
        ...
        {impact.goals.length === 0 && (
          <div className="bg-gradient-to-br from-amber-500/5 to-amber-600/[0.02] rounded-2xl p-4 border border-amber-500/10 text-center">
            ...
            <p className="text-xs text-stone-700">No goals connected to this workout yet.</p>
        ...
        {trackedExercises.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-widest text-stone-700 mb-3">Session Exercises</h3>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <ArrowRight className="w-4 h-4 text-stone-800 rotate-180" />
            </button>
            <h2 className="text-sm font-semibold text-stone-900">Workout in Progress</h2>
          </div>
          <div className="flex items-center gap-2">
            <Timer className="w-3.5 h-3.5 text-stone-700" />
            <span className="text-sm font-bold tabular-nums text-stone-700">{formatTimer(elapsedSeconds)}</span>
            <button
          ...
          <div className="rounded-2xl border border-stone-200 bg-transparent overflow-hidden">
            ...
            <div className="flex items-center justify-between px-3.5 pt-3 pb-1.5">
              <button
                ...
                className="flex items-center gap-1.5 text-[10px] text-stone-700 hover:text-stone-800 transition-colors"
              >
              ...
              <div className="px-3 pb-2 space-y-1">
                {connectedDevices.map(d => (
                  ...
                  >
                    ...
                    {d.battery && <span className="ml-auto text-[9px] text-stone-600">🔋 {d.battery}%</span>}
                  </button>
                ))}
              </div>
            )}
            ...
            {/* Live metrics */}
            <div className="grid grid-cols-4 gap-px bg-transparent">
              {(() => {
                const hrZone = getHRZone(wearableData.heartRate);
                return (
                  <div className="bg-stone-100 p-3 text-center">
                    <p className="text-[9px] text-stone-600 uppercase tracking-wider mb-1">❤️ HR</p>
              ...
              <div className="bg-stone-100 p-3 text-center">
                <p className="text-[9px] text-stone-600 uppercase tracking-wider mb-1">🔥 Cal</p>
                <p className="text-lg font-bold tabular-nums text-orange-400">{wearableData.calories}</p>
                <p className="text-[8px] text-stone-600 mt-0.5">active</p>
              </div>
              {activeWearable.type === 'watch' && wearableData.cadence ? (
                <div className="bg-stone-100 p-3 text-center">
                  <p className="text-[9px] text-stone-600 uppercase tracking-wider mb-1">🦶 Cadence</p>
                  <p className="text-lg font-bold tabular-nums text-blue-400">{wearableData.cadence}</p>
                  <p className="text-[8px] text-stone-600 mt-0.5">spm</p>
                </div>
              ) : (
                <div className="bg-stone-100 p-3 text-center">
                  <p className="text-[9px] text-stone-600 uppercase tracking-wider mb-1">🫁 SpO₂</p>
                  <p className="text-lg font-bold tabular-nums text-cyan-400">{wearableData.bloodOxygen}%</p>
                  <p className="text-[8px] text-stone-600 mt-0.5">oxygen</p>
                </div>
              )}
              <div className="bg-stone-100 p-3 text-center">
                <p className="text-[9px] text-stone-600 uppercase tracking-wider mb-1">😰 Stress</p>
                <p className="text-lg font-bold tabular-nums text-purple-400">{wearableData.stress}</p>
                <p className="text-[8px] text-stone-600 mt-0.5">score</p>
              </div>
            </div>
          </div>
        )}
        ...
        {!hasWearable && connectedDevices.length === 0 && (
          <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-transparent border border-stone-200/70">
            <span className="text-sm">⌚</span>
            <p className="text-[10px] text-stone-600">Connect a wearable in Settings to see live biometrics here</p>
          </div>
        )}
        ...
        {showNextConfirm && (
          <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-r from-amber-500/10 to-amber-600/5 p-4">
            ...
            <p className="text-xs text-stone-700 mb-3">Your current exercise will be marked as done.</p>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-stone-900/5 text-stone-700 text-xs font-semibold hover:bg-stone-900/10 transition-colors"
              >
                Cancel
              </button>
            </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-semibold ${isDone ? 'text-stone-700 line-through' : isActive ? 'text-emerald-300' : 'text-stone-900'}`}>{ex.name}</p>
                        ...
                        {isActive && ex.note && (
                          <p className="text-xs text-stone-700 mt-3 ml-11 leading-relaxed">{ex.note}</p>
                        )}
                      </div>
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    {metrics.filter(m => m.required).map(m => (
                                      <div key={m.key} className="space-y-1">
                                        <label className="text-[10px] text-stone-700 uppercase tracking-wider">{m.label}{m.unit ? ` (${m.unit})` : ''}</label>
                                        <input type="number"
                                          value={
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-stone-700">Recover before next exercise</p>
                            </div>
                          </div>
                          <span className="text-2xl font-bold tabular-nums text-amber-400">
                            {formatTimer(restElapsed)}
                            <span className="text-xs text-stone-700 font-normal"> / {formatTimer(restTargetSec)}</span>
                          </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between rounded-xl bg-stone-900/5 border border-stone-200 px-3 py-2">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="w-7 h-7 rounded-lg bg-stone-900/5 hover:bg-stone-900/10 text-stone-700 text-sm font-bold transition-colors"
          ...
          <div className="rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-stone-900/[0.05] to-stone-900/[0.03] p-4 space-y-3">
            ...
            <div className="max-h-48 overflow-y-auto space-y-1">
              {(() => {
                ...
                if (filtered.length === 0) return <p className="text-xs text-stone-600 text-center py-3">No exercises found</p>;
                return Object.entries(grouped).map(([cat, exercises]) => (
                  <div key={cat}>
                    <p className="text-[10px] text-stone-600 uppercase tracking-wider font-semibold px-1 pt-2 pb-1">{cat}</p>
                    {exercises.slice(0, 8).map(ex => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center gap-3 p-2.5 rounded-xl bg-transparent border border-stone-200/70 hover:bg-transparent transition-all text-left"
                        >
                          <span className="text-lg w-7 text-center">{icon?.emoji || '⚡'}</span>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-medium text-stone-800 truncate">{ex.name}</p>
                            <p className="text-[10px] text-stone-600 capitalize">{ex.category}</p>
                          </div>
                 ...
                 <div className="rounded-2xl border border-emerald-500/20 overflow-hidden bg-emerald-950/40">
                  ...
                  {workoutDuration > 0 && (
                    <div className="px-4 pt-3 -mb-1 flex items-center justify-between">
                      <span className="text-[10px] text-stone-700 uppercase tracking-wider font-semibold flex items-center gap-1.5">
                  ...
                  <div className="px-4 pt-3 pb-1">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-[10px] text-stone-700 uppercase tracking-wider font-semibold flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] text-stone-600">15</span>
                      <input
                        type="range"
                        min={15}
                        max={120}
                      ...
                      />
                      <span className="text-[10px] text-stone-600">120</span>
                    </div>
              ...
              {!hasSessions && !todayPlan?.isRest && (
                <div className="rounded-2xl border border-stone-200 bg-transparent p-5 text-center space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-stone-700 mt-1 max-w-xs mx-auto">
              ...
              <div className="sticky top-0 z-10 -mx-1 px-1 py-2 bg-background/80 backdrop-blur rounded-2xl">
                <div className="flex items-center justify-between gap-2 rounded-2xl border border-blue-500/30 bg-blue-500/10 p-3">
                  <div className="flex flex-col">
                    ...
                    {loggingAddedNames.length > 0 && (
                      <span className="text-[11px] text-stone-700 mt-0.5 line-clamp-1">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-3 py-2 rounded-xl text-xs font-medium text-stone-800 hover:text-stone-900 hover:bg-stone-900/5 transition"
                    >
                      Cancel
                    </button>
                    <button
                ...
                <div className="flex-1 min-w-0 text-left">
                  <div className="flex items-center gap-2">
                    ...
                    {todayPlan.sessions.length > 1 && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-stone-900/5 text-stone-700 shrink-0">Session {si + 1}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] text-stone-600">{sessionExCount} exercise{sessionExCount !== 1 ? 's' : ''}</span>
                    {sessionCompleted > 0 && (
                    ...
                    <div key={ei}>
                      {showRestBefore && (
                        <div className="flex items-center gap-2 py-1 px-2">
                          ...
                          <span className="text-[9px] text-stone-600 flex items-center gap-1">😮‍💨 ~1.5 min rest</span>
                          <div className="flex-1 h-px bg-transparent" />
                        </div>
                      )}
                      <div className="relative">
                        <div
                          className={`w-full flex items-center gap-2 p-2.5 rounded-xl transition-all ${
                            ...
                            : 'bg-transparent border border-stone-200/70 hover:bg-transparent'
                          }`}
                        >
                          <button
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs font-medium truncate ${isDone ? 'text-stone-700' : 'text-stone-800'}`}>{ex.name}</p>
                          ...
                          {editable && !isDone && (
                            <div className="flex items-center gap-0.5 shrink-0">
                              <button
                                ...
                                className="w-7 h-7 flex items-center justify-center rounded-md text-stone-600 hover:text-stone-700 hover:bg-stone-900/10 disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                              >
                              ...
                              <button
                                ...
                                className="w-7 h-7 flex items-center justify-center rounded-md text-stone-600 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                              >
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            </div>
                          )}
                          {!isDone && !editable && <ChevronRight className="w-3.5 h-3.5 text-stone-400 shrink-0" />}
                        </div>
                        ...
                        {isExExpanded && !isDone && (
                          <div className="absolute inset-0 z-10 flex items-center justify-center gap-2 rounded-xl bg-stone-900/40 backdrop-blur-sm border border-stone-200">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-2 rounded-lg bg-stone-900/5 text-stone-700 text-[11px] font-semibold hover:bg-red-500/15 hover:text-red-400 transition-colors"
                            >
                              ✕ Skip
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); setExpandedExercise(null); }}
                              className="flex items-center gap-1 px-3 py-2 rounded-lg bg-stone-900/5 text-stone-700 text-[11px] font-semibold hover:bg-stone-900/10 transition-colors"
                            >
                              Cancel
                            </button>
                          </div>
...
const PastWorkoutsList = ({ completedWorkouts, workouts, onDeleteLocal, onDeleteByDate }: PastWorkoutsListProps) => {
  ...
  return (
    <div className="space-y-3 pt-2">
      <p className="text-xs font-semibold text-stone-700 text-center">Past Workouts</p>
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-stone-700 hover:text-red-400 hover:bg-red-500/10 transition-colors"
            ...
            {isExpanded && (
              <div className="px-3.5 pb-3.5 space-y-1.5 border-t border-stone-200 pt-3">
                {item.exercises.map((ex, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 p-2.5 rounded-xl bg-transparent border border-stone-200/70">
                      <span className="text-base w-6 text-center">{exIcon.emoji}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-stone-700">{ex.name}</p>
                        <p className="text-[10px] text-stone-600">
                          {[
                ...
                {item.exercises.length === 0 && (
                  <p className="text-xs text-stone-600 text-center py-2">No exercise details recorded</p>
                )}
              </div>
            )}
          </div>
  ...
  if (workouts.length === 0 && activeGoals.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-sm text-stone-700">No workouts logged yet</p>
        <p className="text-xs text-stone-700 mt-1">Complete a workout to see your history here.</p>
      </div>
    );
  }
      ...
      {activeGoals.length > 0 && (
        <div className="bg-gradient-to-br from-stone-900/[0.05] to-stone-900/[0.02] rounded-2xl border border-stone-200 overflow-hidden">
          <div className="p-4 pb-2">
            <div className="flex items-center gap-2 mb-3">
              <TrendingUp className="w-4 h-4 text-blue-400" />
              <span className="text-xs font-semibold uppercase tracking-widest text-stone-700">Goal Burndown</span>
            </div>
            ...
            {selectedGoal && (
              <div className="mt-2 flex items-center gap-3 text-[10px] text-stone-600">
      ...
      {Object.entries(grouped).map(([date, dayWorkouts]) => (
        <div key={date}>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold text-stone-700">{formatDate(date)}</span>
            <span className="text-[10px] text-stone-700">· {dayWorkouts.reduce((s, w) => s + w.score, 0)} pts</span>
          </div>
          <div className="space-y-2">
            {dayWorkouts.map(w => {
              ...
              return (
                <div key={w.id} className={`flex items-center gap-3 bg-gradient-to-r ${cfg.gradient} rounded-2xl p-3.5 border ${cfg.border}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-stone-700 capitalize">{w.type}</p>
                  </div>
