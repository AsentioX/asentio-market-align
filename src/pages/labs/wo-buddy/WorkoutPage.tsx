      ...
      <div className="space-y-6">
        <div className="flex flex-col items-center justify-center text-center space-y-5 pt-2">
          ...
          <div>
            ...
            <p className="text-stone-900/65 text-sm">
              {elapsedSeconds > 0 && <span className="text-stone-900/75">{formatTimer(elapsedSeconds)} · </span>}
          ...
          <div className="bg-gradient-to-br from-stone-900/[0.06] to-stone-900/[0.03] rounded-2xl p-6 border border-stone-900/10 w-full max-w-xs">
            ...
            <p className="text-xs text-stone-900/65 mt-1">points earned</p>
            <div className="mt-4 flex items-start gap-1.5 text-[11px] text-stone-900/55 bg-stone-900/[0.04] rounded-xl p-3">
        ...
        {impact.drivers.length > 0 && (
          <div className="bg-gradient-to-br from-stone-900/[0.05] to-stone-900/[0.03] rounded-2xl p-4 border border-stone-900/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-stone-900/70">Drivers Activated</span>
            </div>
                ...
                return (
                  <div key={d} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-stone-900/5 border border-stone-900/10">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-stone-900/55">{info?.description || ''}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {impact.goals.length > 0 && (
          <div className="bg-gradient-to-br from-emerald-500/5 to-emerald-600/[0.02] rounded-2xl p-4 border border-emerald-500/10">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-semibold uppercase tracking-widest text-stone-900/70">Goal Impact</span>
            </div>
                ...
                return (
                  <div key={g.id} className="bg-stone-900/[0.04] rounded-xl p-3 border border-stone-900/10">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-stone-900/55">{g.drivers.join(', ')}</p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-stone-900/70 font-medium tabular-nums">{pct}%</span>
                    </div>
        ...
        {impact.goals.length === 0 && (
          <div className="bg-gradient-to-br from-amber-500/5 to-amber-600/[0.02] rounded-2xl p-4 border border-amber-500/10 text-center">
            ...
            <p className="text-xs text-stone-900/70">No goals connected to this workout yet.</p>
        ...
        {trackedExercises.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold uppercase tracking-widest text-stone-900/70 mb-3">Session Exercises</h3>
            ...
            <div className="mt-3 text-center text-xs text-stone-900/55">
        ...
        <div className="flex items-center gap-4 justify-center pb-4">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-sm text-stone-900/70 hover:text-stone-900 transition-colors"
          >
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            >
              <ArrowRight className="w-4 h-4 text-stone-900/75 rotate-180" />
            </button>
            <h2 className="text-sm font-semibold text-stone-900">Workout in Progress</h2>
          </div>
          <div className="flex items-center gap-2">
            <Timer className="w-3.5 h-3.5 text-stone-900/65" />
          ...
          <div className="rounded-2xl border border-stone-900/10 bg-stone-900/[0.04] overflow-hidden">
            ...
            <div className="flex items-center justify-between px-3.5 pt-3 pb-1.5">
              <button
                ...
                className="flex items-center gap-1.5 text-[10px] text-stone-900/65 hover:text-stone-900/75 transition-colors"
              >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl text-left text-xs transition-colors ${
                      d.id === selectedWearableId
                        ...
                        : 'bg-stone-900/[0.04] text-stone-900/75 border border-stone-900/10 hover:bg-stone-900/[0.06]'
                    }`}
                  >
                    <span>{d.type === 'watch' ? '⌚' : d.type === 'ring' ? '💍' : '📱'}</span>
                    <span className="font-medium">{d.name}</span>
                    {d.battery && <span className="ml-auto text-[9px] text-stone-900/55">🔋 {d.battery}%</span>}
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="grid grid-cols-4 gap-px bg-stone-900/[0.05]">
              {(() => {
                ...
                return (
                  <div className="bg-stone-100 p-3 text-center">
                    <p className="text-[9px] text-stone-900/55 uppercase tracking-wider mb-1">❤️ HR</p>
              ...
              <div className="bg-stone-100 p-3 text-center">
                <p className="text-[9px] text-stone-900/55 uppercase tracking-wider mb-1">🔥 Cal</p>
                <p className="text-lg font-bold tabular-nums text-orange-400">{wearableData.calories}</p>
                <p className="text-[8px] text-stone-900/45 mt-0.5">active</p>
              </div>
              {activeWearable.type === 'watch' && wearableData.cadence ? (
                <div className="bg-stone-100 p-3 text-center">
                  <p className="text-[9px] text-stone-900/55 uppercase tracking-wider mb-1">🦶 Cadence</p>
                  <p className="text-lg font-bold tabular-nums text-blue-400">{wearableData.cadence}</p>
                  <p className="text-[8px] text-stone-900/45 mt-0.5">spm</p>
                </div>
              ) : (
                <div className="bg-stone-100 p-3 text-center">
                  <p className="text-[9px] text-stone-900/55 uppercase tracking-wider mb-1">🫁 SpO₂</p>
                  <p className="text-lg font-bold tabular-nums text-cyan-400">{wearableData.bloodOxygen}%</p>
                  <p className="text-[8px] text-stone-900/45 mt-0.5">oxygen</p>
                </div>
              )}
              <div className="bg-stone-100 p-3 text-center">
                <p className="text-[9px] text-stone-900/55 uppercase tracking-wider mb-1">😰 Stress</p>
                <p className="text-lg font-bold tabular-nums text-purple-400">{wearableData.stress}</p>
                <p className="text-[8px] text-stone-900/45 mt-0.5">score</p>
              </div>
            </div>
          </div>
        )}
        ...
        {!hasWearable && connectedDevices.length === 0 && (
          <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-stone-900/[0.03] border border-stone-900/[0.08]">
            ...
            <p className="text-[10px] text-stone-900/55">Connect a wearable in Settings to see live biometrics here</p>
          </div>
        )}
        ...
        {showNextConfirm && (
          <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-r from-amber-500/10 to-amber-600/5 p-4">
            ...
            <p className="text-xs text-stone-900/65 mb-3">Your current exercise will be marked as done.</p>
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-stone-900/5 text-stone-900/70 text-xs font-semibold hover:bg-stone-900/10 transition-colors"
              >
                Cancel
              </button>
            </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className={`text-sm font-semibold ${isDone ? 'text-stone-900/65 line-through' : isActive ? 'text-emerald-300' : 'text-stone-900'}`}>{ex.name}</p>
                        ...
                        {isActive && ex.note && (
                          <p className="text-xs text-stone-900/65 mt-3 ml-11 leading-relaxed">{ex.note}</p>
                        )}
                      </div>
                          ...
                          {!inputMode && (
                            <div className="grid grid-cols-3 gap-2 pt-2">
                              ...
                              >
                                ...
                                <span className="text-[10px] text-stone-900/75 font-medium">Camera</span>
                              </button>
                              <button
                              ...
                              >
                                ...
                                <span className="text-[10px] text-stone-900/75 font-medium">Photo</span>
                              </button>
                              <button
                              ...
                              >
                                ...
                                <span className="text-[10px] text-stone-900/75 font-medium">Manual</span>
                              </button>
                            </div>
                          )}
                          ...
                          {inputMode === 'camera' && (
                            <div className="space-y-2 pt-2">
                              <div className="flex items-center justify-between">
                                ...
                                <button onClick={() => setExerciseInputMode(prev => ({ ...prev, [key]: null }))} className="text-[10px] text-stone-900/55 hover:text-stone-900/70">Back</button>
                              </div>
                              <CameraTrackingView
                                exercise={ex.name}
                          ...
                          {inputMode === 'photo' && (
                            <div className="space-y-2 pt-2">
                              ...
                              <div className="rounded-xl border border-stone-900/10 bg-stone-900/[0.04] p-6 text-center">
                                ...
                                <p className="text-xs text-stone-900/65">Take a photo of your exercise machine or setup</p>
                                  ...
                                  return (
                                    <div className={`grid ${hasWeight ? 'grid-cols-3' : 'grid-cols-2'} gap-2`}>
                                      <div className="space-y-1">
                                        <label className="text-[10px] text-stone-900/65 uppercase tracking-wider">Sets</label>
                                      ...
                                      <div className="space-y-1">
                                        <label className="text-[10px] text-stone-900/65 uppercase tracking-wider">{repsLabel}</label>
                                      ...
                                      {hasWeight && (
                                        <div className="space-y-1">
                                          <label className="text-[10px] text-stone-900/65 uppercase tracking-wider">Weight</label>
                                ...
                                if (entryType === 'intervals') {
                                  ...
                                  return (
                                    <div className="grid grid-cols-3 gap-2">
                                      <div className="space-y-1">
                                        <label className="text-[10px] text-stone-900/65 uppercase tracking-wider">Distance</label>
                                      ...
                                      <div className="space-y-1">
                                        <label className="text-[10px] text-stone-900/65 uppercase tracking-wider">Time</label>
                                ...
                                if (entryType === 'duration') {
                                  ...
                                  return (
                                    <div className="grid grid-cols-2 gap-2">
                                      <div className="space-y-1">
                                        <label className="text-[10px] text-stone-900/65 uppercase tracking-wider">Duration</label>
                                  ...
                                  <div className="grid grid-cols-2 gap-2">
                                    {metrics.filter(m => m.required).map(m => (
                                      <div key={m.key} className="space-y-1">
                                        <label className="text-[10px] text-stone-900/65 uppercase tracking-wider">{m.label}{m.unit ? ` (${m.unit})` : ''}</label>
                                        <input type="number"
                                          value={
                    ...
                    {showRestAfter && (
                      <div className="rounded-2xl border border-amber-500/20 bg-gradient-to-r from-amber-500/10 to-amber-600/5 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-stone-900/65">Recover before next exercise</p>
                            </div>
                          </div>
                          <span className="text-2xl font-bold tabular-nums text-amber-400">
                            {formatTimer(restElapsed)}
                            <span className="text-xs text-stone-900/65 font-normal"> / {formatTimer(restTargetSec)}</span>
                          </span>
                        </div>
                        ...
                        <div className="flex items-center justify-between rounded-xl bg-stone-900/5 border border-stone-900/10 px-3 py-2">
                          <span className="text-[10px] uppercase tracking-widest text-stone-900/65 font-semibold">Target rest</span>
                          <div className="flex items-center gap-2">
                            <button
                              type="button"
        ...
        {!showAddExercise ? (
          ...
          >
            <Plus className="w-4 h-4 text-stone-900/70" />
          ...
          <div className="rounded-2xl border border-emerald-500/20 bg-gradient-to-br from-stone-900/[0.05] to-stone-900/[0.03] p-4 space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-widest text-stone-900/70">Add Exercise</span>
              <button onClick={() => { setShowAddExercise(false); setAddExerciseSearch(''); }} className="text-[10px] text-stone-900/55 hover:text-stone-900/70">Cancel</button>
            </div>
            <input
              type="text"
              ...
              className="w-full bg-stone-900/5 border border-stone-900/10 rounded-xl px-3 py-2.5 text-sm text-stone-900 placeholder:text-stone-900/55 focus:outline-none focus:border-emerald-500/30"
              autoFocus
            />
            <div className="max-h-48 overflow-y-auto space-y-1">
              {(() => {
                ...
                if (filtered.length === 0) return <p className="text-xs text-stone-900/55 text-center py-3">No exercises found</p>;
                return Object.entries(grouped).map(([cat, exercises]) => (
                  <div key={cat}>
                    <p className="text-[10px] text-stone-900/55 uppercase tracking-wider font-semibold px-1 pt-2 pb-1">{cat}</p>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-stone-900/55 capitalize">{ex.category}</p>
                          </div>
            ...
            <div className="space-y-4">
                 ...
                 <div className="rounded-2xl border border-emerald-500/20 overflow-hidden bg-emerald-950/40">
                  ...
                  {workoutDuration > 0 && (
                    <div className="px-4 pt-3 -mb-1 flex items-center justify-between">
                      <span className="text-[10px] text-stone-900/65 uppercase tracking-wider font-semibold flex items-center gap-1.5">
                  ...
                  <div className="px-4 pt-3 pb-1">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-[10px] text-stone-900/65 uppercase tracking-wider font-semibold flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-[10px] text-stone-900/55">15</span>
                      <input
                        type="range"
                        min={15}
                        max={120}
                      ...
                      />
                      <span className="text-[10px] text-stone-900/55">120</span>
                    </div>
              ...
              {todayPlan?.isRest && todayPlan.sessions.length === 0 && (
                <div className="rounded-2xl bg-stone-900/[0.04] border border-stone-900/10 p-4 text-center">
                  ...
                  <p className="text-sm text-stone-900/70 mt-2">Today is a rest day</p>
                  <p className="text-[11px] text-stone-900/55 mt-1">{todayPlan.restReason}</p>
                </div>
              )}
              ...
              {!hasSessions && !todayPlan?.isRest && (
                <div className="rounded-2xl border border-stone-900/10 bg-stone-900/[0.04] p-5 text-center space-y-3">
                  <div className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-stone-900/[0.05] border border-stone-900/10">
                    <CalendarDays className="w-4 h-4 text-stone-900/65" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-stone-900">No plan for today</p>
                    <p className="text-[11px] text-stone-900/65 mt-1 max-w-xs mx-auto">
              ...
              <div className="sticky top-0 z-10 -mx-1 px-1 py-2 bg-background/80 backdrop-blur rounded-2xl">
                <div className="flex items-center justify-between gap-2 rounded-2xl border border-blue-500/30 bg-blue-500/10 p-3">
                  <div className="flex flex-col">
                    ...
                    {loggingAddedNames.length > 0 && (
                      <span className="text-[11px] text-stone-900/70 mt-0.5 line-clamp-1">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="px-3 py-2 rounded-xl text-xs font-medium text-stone-900/75 hover:text-stone-900/90 hover:bg-stone-900/5 transition"
                    >
                      Cancel
                    </button>
                    <button
...
const SESSION_TYPE_STYLES: Record<string, { emoji: string; color: string; gradient: string; border: string; activeGradient: string }> = {
  ...
  rest: { emoji: '😴', color: 'text-stone-900/65', gradient: 'from-stone-900/5 to-stone-900/[0.03]', border: 'border-stone-900/10', activeGradient: 'from-stone-900/10 to-stone-900/5' },
};
                ...
                <div className="flex-1 min-w-0 text-left">
                  <div className="flex items-center gap-2">
                    ...
                    {todayPlan.sessions.length > 1 && (
                      <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-stone-900/5 text-stone-900/65 shrink-0">Session {si + 1}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-[10px] text-stone-900/55">{sessionExCount} exercise{sessionExCount !== 1 ? 's' : ''}</span>
                    {sessionCompleted > 0 && (
                    ...
                    <div key={ei}>
                      {showRestBefore && (
                        <div className="flex items-center gap-2 py-1 px-2">
                          ...
                          <span className="text-[9px] text-stone-900/45 flex items-center gap-1">😮‍💨 ~1.5 min rest</span>
                          <div className="flex-1 h-px bg-stone-900/[0.06]" />
                        </div>
                      )}
                      <div className="relative">
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs font-medium truncate ${isDone ? 'text-stone-900/65' : 'text-stone-900/80'}`}>{ex.name}</p>
                          ...
                          {editable && !isDone && (
                            <div className="flex items-center gap-0.5 shrink-0">
                              <button
                                ...
                                className="w-7 h-7 flex items-center justify-center rounded-md text-stone-900/55 hover:text-stone-900/70 hover:bg-stone-900/10 disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                              >
                              ...
                              <button
                                ...
                                className="w-7 h-7 flex items-center justify-center rounded-md text-stone-900/55 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                              >
                        ...
                        {isExExpanded && !isDone && (
                          <div className="absolute inset-0 z-10 flex items-center justify-center gap-2 rounded-xl bg-stone-900/40 backdrop-blur-sm border border-stone-900/10">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-3 py-2 rounded-lg bg-stone-900/5 text-stone-900/65 text-[11px] font-semibold hover:bg-red-500/15 hover:text-red-400 transition-colors"
                            >
                              ✕ Skip
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); setExpandedExercise(null); }}
                              className="flex items-center gap-1 px-3 py-2 rounded-lg bg-stone-900/5 text-stone-900/65 text-[11px] font-semibold hover:bg-stone-900/10 transition-colors"
                            >
                              Cancel
                            </button>
                          </div>
...
const PastWorkoutsList = ({ completedWorkouts, workouts, onDeleteLocal, onDeleteByDate }: PastWorkoutsListProps) => {
  ...
  return (
    <div className="space-y-3 pt-2">
      <p className="text-xs font-semibold text-stone-900/70 text-center">Past Workouts</p>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-[10px] text-stone-900/65">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-stone-900/65 hover:text-red-400 hover:bg-red-500/10 transition-colors"
                  aria-label="Delete workout"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
                <ChevronDown className={`w-4 h-4 text-stone-900/55 transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
              </div>
            </button>
            ...
            {isExpanded && (
              <div className="px-3.5 pb-3.5 space-y-1.5 border-t border-stone-900/10 pt-3">
                ...
                {item.exercises.length === 0 && (
                  <p className="text-xs text-stone-900/55 text-center py-2">No exercise details recorded</p>
                )}
              </div>
            )}
          </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <div className="w-16 h-16 rounded-2xl bg-stone-900/5 flex items-center justify-center mb-4">
          <History className="w-8 h-8 text-stone-900/45" />
        </div>
        <p className="text-sm text-stone-900/65">No workouts logged yet</p>
        <p className="text-xs text-stone-900/70 mt-1">Complete a workout to see your history here.</p>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-gradient-to-b from-stone-900/[0.05] to-stone-900/[0.03] rounded-xl p-3 border border-stone-900/10 text-center">
          ...
          <p className="text-[10px] text-stone-900/65 mt-0.5">Total</p>
        </div>
        <div className="bg-gradient-to-b from-stone-900/[0.05] to-stone-900/[0.03] rounded-xl p-3 border border-stone-900/10 text-center">
          <p className="text-xl font-bold text-emerald-400">{workouts.reduce((s, w) => s + w.score, 0).toLocaleString()}</p>
          <p className="text-[10px] text-stone-900/65 mt-0.5">Points</p>
        </div>
        <div className="bg-gradient-to-b from-stone-900/[0.05] to-stone-900/[0.03] rounded-xl p-3 border border-stone-900/10 text-center">
          <p className="text-xl font-bold">{Object.keys(grouped).length}</p>
          <p className="text-[10px] text-stone-900/65 mt-0.5">Days</p>
        </div>
      </div>
      ...
      {activeGoals.length > 0 && (
        <div className="bg-gradient-to-br from-stone-900/[0.05] to-stone-900/[0.02] rounded-2xl border border-stone-900/10 overflow-hidden">
          <div className="p-4 pb-2">
            <div className="flex items-center gap-2 mb-3">
              <TrendingUp className="w-4 h-4 text-blue-400" />
              <span className="text-xs font-semibold uppercase tracking-widest text-stone-900/70">Goal Burndown</span>
            </div>
                ...
                {activeGoals.map(g => (
                  <button key={g.id} onClick={() => setSelectedGoalId(g.id)}
                    className={`shrink-0 text-[11px] px-2.5 py-1 rounded-lg border transition-colors ${
                      (selectedGoal?.id === g.id) ? 'bg-blue-500/15 border-blue-500/20 text-blue-400' : 'bg-stone-900/[0.04] border-stone-900/10 text-stone-900/65 hover:bg-stone-900/[0.06]'
                    }`}>{g.name}</button>
                ))}
              </div>
            )}
            {selectedGoal && (
              <div className="mt-2 flex items-center gap-3 text-[10px] text-stone-900/55">
          ...
          {drift > 0 && selectedGoal && (
            <div className="mx-4 mb-4 rounded-xl bg-amber-500/[0.08] border border-amber-500/10 p-3 flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-stone-900/65 mt-0.5">
      ...
      {Object.entries(grouped).map(([date, dayWorkouts]) => (
        <div key={date}>
          <div className="flex items-center gap-2 mb-2">
            <Calendar className="w-3 h-3 text-stone-900/55" />
            <span className="text-xs font-semibold text-stone-900/70">{formatDate(date)}</span>
            <span className="text-[10px] text-stone-900/70">· {dayWorkouts.reduce((s, w) => s + w.score, 0)} pts</span>
          </div>
          <div className="space-y-2">
            {dayWorkouts.map(w => {
              ...
              return (
                <div key={w.id} className={`flex items-center gap-3 bg-gradient-to-r ${cfg.gradient} rounded-2xl p-3.5 border ${cfg.border}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-stone-900/65 capitalize">{w.type}</p>
                  </div>
...
function NumberInput({ label, value, onChange, step = 1 }: { label: string; value: number; onChange: (v: number) => void; step?: number }) {
  return (
    <div className="min-w-0">
      <label className="text-[10px] text-stone-900/65 uppercase tracking-wider mb-1 block">{label}</label>
      <div className="flex items-center gap-0.5">
        <button onClick={() => onChange(Math.max(0, value - step))} className="w-7 h-8 shrink-0 rounded-lg bg-stone-900/5 text-stone-900/75 hover:bg-stone-900/10 flex items-center justify-center text-sm transition-colors">−</button>
        ...
        <button onClick={() => onChange(value + step)} className="w-7 h-8 shrink-0 rounded-lg bg-stone-900/5 text-stone-900/75 hover:bg-stone-900/10 flex items-center justify-center text-sm transition-colors">+</button>
      </div>
    </div>
  );
}
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        ...
        {groupedOptions ? (
          Object.entries(groupedOptions).map(([cat, names]) => (
            <optgroup key={cat} label={cat} className="bg-white text-stone-900/75">
