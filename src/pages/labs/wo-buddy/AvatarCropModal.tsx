...
const AvatarCropModal = ({ imageSrc, onCropDone, onCancel }: AvatarCropModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      <div className="bg-[#111118] rounded-2xl border border-white/10 w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
          <span className="text-sm font-medium text-white/80">Crop Profile Photo</span>
        ...
        <div className="flex gap-2 px-4 pb-4">
          <button onClick={onCancel} className="flex-1 text-sm py-2.5 rounded-xl border border-white/10 text-white/60 hover:bg-white/5 transition-colors">
            Cancel
          </button>
          <button
            onClick={handleSave}
