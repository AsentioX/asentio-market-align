    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {competitions.map((comp) => {
          ...
          return (
            <div key={comp.id} className="rounded-2xl border border-white/[0.08] overflow-hidden shadow-lg shadow-black/10">
              ...
              <div className="relative h-36">
                ...
                <div className="absolute top-3 left-3">
                  <span className={`text-[10px] px-2.5 py-1 rounded-full font-medium backdrop-blur-md border border-white/10 ${
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  ...
                  <p className="text-[11px] text-white/70 mt-0.5">{comp.description}</p>
                </div>
              </div>
              ...
              <div className="bg-white/[0.03] p-4 space-y-3">
                ...
                <div className="flex items-center justify-between text-[10px] text-white/40">
                  <span className="font-medium text-white/60">{comp.progress} / {comp.target}</span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-2.5 rounded-xl bg-white/5 text-white/40 hover:text-white/60 border border-white/5 transition-colors"
                  >
              ...
              {activeLeaderboard === comp.id && (
                <div className="border-t border-white/5 p-4 space-y-1.5 bg-white/[0.02]">
                  <p className="text-[10px] text-white/30 uppercase tracking-wider mb-2">Leaderboard</p>
                  {mockLeaderboard.map((entry) => (
                    <div
                      key={entry.rank}
                      className={`flex items-center gap-3 p-2.5 rounded-xl transition-colors ${
                        entry.isCurrentUser ? 'bg-emerald-500/10 border border-emerald-500/20' : 'hover:bg-white/[0.03]'
                      }`}
                    >
                      ...
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold ${
                        entry.isCurrentUser ? 'bg-gradient-to-br from-emerald-500 to-emerald-700' : 'bg-white/10'
                      }`}>
                        {entry.avatar}
                      </div>
