    ...
    <div className="bg-gradient-to-r from-amber-500/8 to-amber-600/3 rounded-xl border border-amber-500/10 overflow-hidden">
      <button onClick={() => setExpanded(!expanded)}
        ...
        <div className="flex-1 min-w-0">
          ...
          {primary && !expanded && (
            <p className="text-[11px] text-white/50 truncate mt-0.5">{primary.training_purpose}</p>
          )}
        </div>
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-2.5">
          {primary && (
            <>
              <div>
                <span className="text-[9px] text-white/30 uppercase tracking-wider">Purpose</span>
                <p className="text-xs text-white/60 mt-0.5">{primary.training_purpose}</p>
              </div>
              <div>
                <span className="text-[9px] text-white/30 uppercase tracking-wider">How it helps</span>
                <p className="text-xs text-white/60 mt-0.5">{primary.explanation}</p>
              </div>
              {primary.target_suggestion && (
                <div>
                  <span className="text-[9px] text-white/30 uppercase tracking-wider">Today's focus</span>
                  <p className="text-xs text-white/60 mt-0.5">{primary.target_suggestion}</p>
                </div>
              )}
            </>
          )}
          ...
          {drivers.length > 0 && (
            <div className="flex flex-wrap gap-1">
              {drivers.map(d => {
                ...
                return (
                  <span key={d} className="flex items-center gap-1 px-2 py-0.5 rounded-md bg-white/5 text-[9px] text-white/40 border border-white/5">
                    {info?.icon} {d}
                  </span>
                );
              })}
          ...
          {connectedGoals.length > 0 && (
            <div>
              <span className="text-[9px] text-white/30 uppercase tracking-wider">Goal connection</span>
