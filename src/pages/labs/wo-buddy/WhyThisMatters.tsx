    ...
    <div className="bg-gradient-to-r from-amber-500/8 to-amber-600/3 rounded-xl border border-amber-500/10 overflow-hidden">
      <button onClick={() => setExpanded(!expanded)}
        ...
        <div className="flex-1 min-w-0">
          ...
          {primary && !expanded && (
            <p className="text-[11px] text-stone-700 truncate mt-0.5">{primary.training_purpose}</p>
          )}
        </div>
        {expanded ? <ChevronUp className="w-3 h-3 text-stone-600" /> : <ChevronDown className="w-3 h-3 text-stone-600" />}
      </button>
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-2.5">
          {primary && (
            <>
              <div>
                <span className="text-[9px] text-stone-600 uppercase tracking-wider">Purpose</span>
                <p className="text-xs text-stone-800 mt-0.5">{primary.training_purpose}</p>
              </div>
              <div>
                <span className="text-[9px] text-stone-600 uppercase tracking-wider">How it helps</span>
                <p className="text-xs text-stone-800 mt-0.5">{primary.explanation}</p>
              </div>
              {primary.target_suggestion && (
                <div>
                  <span className="text-[9px] text-stone-600 uppercase tracking-wider">Today's focus</span>
                  <p className="text-xs text-stone-800 mt-0.5">{primary.target_suggestion}</p>
                </div>
              )}
            </>
          )}
          ...
          {connectedGoals.length > 0 && (
            <div>
              <span className="text-[9px] text-stone-600 uppercase tracking-wider">Goal connection</span>
