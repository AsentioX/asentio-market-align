...
import { Wallet, CreditCard, X, Plus } from 'lucide-react';
    ...
    <div className="px-5 pt-5 pb-4">
      ...
      {memberships.length === 0 ? (
        ...
        >
          ...
          <p className="text-xs text-slate-500 max-w-[260px]">Snap a photo of any membership or credit card to keep it handy.</p>
        </button>
      ) : (
            ...
            <div key={m.id}>
              <AnimatePresence mode="wait">
                {flipped === m.id ? (
                  ...
                  >
                    <button onClick={() => setFlipped(null)} className="absolute top-4 right-4 text-white/70 hover:text-white z-10">
                      <X className="w-5 h-5" />
                    </button>
                    <div>
                      ...
                      <div className="w-full h-12 bg-white/20 rounded-xl flex items-center justify-center gap-2">
                        <div className="flex gap-[2px]">
                          {Array.from({ length: 30 }).map((_, i) => (
                            <div key={i} className="w-[2px] bg-white/80 rounded-full" style={{ height: `${12 + ((i * 7) % 20)}px` }} />
                          ))}
                        </div>
                      </div>
                  ...
                  <motion.button
                    ...
                    className="w-full rounded-3xl text-white text-left relative overflow-hidden min-h-[140px] active:scale-[0.98] transition-transform"
                    style={{ backgroundColor: m.brandColor }}
                  >
                    {m.cardImageUrl ? (
                      <>
