...
import { useDrafts } from '@/hooks/useGovernance';
import { Plus, Pencil, Trash2, X, Calendar, Users, FileText, Link as LinkIcon, Loader2 } from 'lucide-react';
...
const MeetingMinutes = () => {
  ...
  const handleSubmit = () => {
    ...
    if (editingId) {
      updateMinute.mutate({ id: editingId, title, meeting_date: meetingDate, attendees, notes, transcript_id: transcriptId });
    } else {
      addMinute.mutate({ title, meeting_date: meetingDate, attendees, notes, transcript_id: transcriptId });
    }
    resetForm();
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-teal-200 p-6 shadow-sm space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">Attendees (comma-separated)</label>
            <input
              value={attendeesStr}
              onChange={(e) => setAttendeesStr(e.target.value)}
              placeholder="Scott Greenwald, Sultan Sharrief, Jon Li…"
              className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"
            />
          </div>
