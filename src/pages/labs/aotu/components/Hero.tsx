...
const Hero = () => {
  return (
    <section className="relative overflow-hidden border-b border-[#E5E7EB]">
      ...
      {/* Soft accent glow */}
      <div
        className="absolute -top-40 -right-40 w-[600px] h-[600px] rounded-full opacity-20 pointer-events-none blur-3xl"
        style={{ background: "radial-gradient(circle, #00B8FF 0%, transparent 60%)" }}
      />
      <div className="relative max-w-7xl mx-auto px-6 pt-24 pb-28 md:pt-32 md:pb-36">
        ...
        <h1
          className="text-[44px] sm:text-[62px] md:text-[84px] leading-[0.95] font-semibold tracking-tight text-[#0A0F1C] max-w-5xl"
          style={{ fontFamily: '"Space Grotesk", sans-serif', letterSpacing: "-0.02em" }}
        >
          Replace manual monitoring with{" "}
          <span className="relative inline-block">
            <span className="relative z-10">AI operators</span>
            <span className="absolute bottom-1 md:bottom-2 left-0 right-0 h-3 md:h-5 bg-[#00B8FF]/70 -z-0" />
          </span>
          .
        </h1>

        <p className="mt-8 max-w-xl text-[18px] md:text-[20px] leading-[1.5] text-[#0A0F1C]/65">
        ...
        <div className="mt-10 flex flex-wrap gap-3">
          <Link
            ...
            className="group inline-flex items-center gap-2 bg-[#0A0F1C] text-white px-6 py-3.5 rounded-md text-[14px] font-semibold hover:bg-[#00B8FF] hover:text-[#0A0F1C] transition-colors"
          >
            Request demo
