...
- **Card color themes**: each card can be given a color from a fixed palette of ~10 named themes (plus "Default"), picked from a swatch row in the card's context menu and in the detail drawer. The theme drives the card's accent bar, border tint, glow, and header wash — always as semantic tokens, so it stays legible in every environment mode. A color legend/filter lets users filter the matrix by theme.
- **Inherited child color**: sub-tasks inherit their parent's theme by default and render the same accent hue at reduced intensity, plus a connecting accent-colored spine from parent to child, so a family of cards is instantly recognizable. A child can be overridden to its own theme; changing a parent's color re-tints all children that are still inheriting.
...
- Color themes: a nullable `color_theme` text column on `tdz_projects` storing a palette key (null = inherit from parent, or default at top level). Palette keys map to CSS custom properties defined in the design system (`--tdz-theme-<key>`), never hardcoded Tailwind color classes, so cards theme correctly across AR / Slate / Studio modes.
